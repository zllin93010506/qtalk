...
public class OpusDecoderTransform implements IAudioTransform 
{
    public  final static String CODEC_NAME = "OPUS";
    public  final static int    DEFAULT_ID = 111;
    private final static String TAG = "OpusDecoderTransform";
    private static final String DEBUGTAG = "QTFa_OpusDecoderTransform";
    private static final int  mInFormat = myAudioFormat.OPUS;
    ...
    protected OpusDecoderTransform()
    {}
    @Override
    ...
    public void onMedia(final ByteBuffer data,final int length,final long timestamp) 
    {
        try
        {
            ...
            {
                if (data.isDirect())
                    _OnMediaDirect(mHandle,data, length, timestamp ,mOutputArray);
                else
                    _OnMedia(mHandle,data.array(), length, timestamp ,mOutputArray);
            }
    ...
    public void setFormat(int format, int sampleRate,int numChannels) throws FailedOperateException ,UnSupportException 
    {
        switch(format)
        {
        case mInFormat://AudioFormat.OPUS
            break;
        default:
