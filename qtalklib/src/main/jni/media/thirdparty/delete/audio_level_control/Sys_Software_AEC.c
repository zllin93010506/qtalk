...
{	
 ...
 void sw_aec_add_dB(int dB_value, int len, short *ptr_data)
 {
         int i=0;
...
void sw_aec_ALC(short *x, int len)
{
	...
	//	time_cnt++;
	if(time_cnt == 75){
		printf("rms=%fdB, highcnt=%d, lowcnt=%d, lock_flag=%d, lock_cnt=%d, level_cnt=%d\n", x_rms, alc_high_cnt, alc_low_cnt, b_alc_gain_lock, alc_lock_cnt, alc_gain_level_cnt);
		time_cnt = 0;
	}
