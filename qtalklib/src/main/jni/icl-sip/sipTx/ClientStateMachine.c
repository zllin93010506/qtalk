 ...
 *
 * $Id: ClientStateMachine.c,v 1.4 2008/11/19 02:48:13 tyhuang Exp $
 */
...
unsigned int g_MaxTxTimeout=32;

void sipTxSetMaxTxTimeout(unsigned int newmaxtime)
{
	if(newmaxtime<1) {g_MaxTxTimeout=1;return;}

	if(newmaxtime>32) {g_MaxTxTimeout=32;return;}
	else
		g_MaxTxTimeout=newmaxtime;
}
...
void Client_Invite_SM(TxStruct TxData, SipTxEvtType EvtType, SipRsp msg)
{
	...
	case TX_CALLING:
		if (TX_TIMER_A == EvtType) {
			...
			TxData->TimerF+=(TxData->TimerA*2);
			/*if ( TxData->TimerA == (32*T1) ) {*/
			if(TxData->TimerF > ((g_MaxTxTimeout*1000)-500) ){
				/* no more arm TimerA */
				TxData->TimerHandle1 = -1;
				/* Arm TimerB */
				TxData->TimerB = T1;
...
void Client_2XX_SM(TxStruct TxData, SipTxEvtType EvtType, SipReq msg)
{
	...
	case TX_COMPLETED:
		...
		if (TX_TIMER_G == EvtType) {
			...
			/* add by tyhuang  2008/11/17 */
#ifdef ICL_SIP_SERVER
				/* no more arm TimerG */
