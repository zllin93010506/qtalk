 ...
 * dx_str.c
 *
 * $Id: dx_str.c,v 1.1.1.1 2008/08/01 07:00:24 judy Exp $
 */
/** 
* @file dx_str.c
* @brief Define dx string related functions
...
{
	DxStr _this;
	_this = malloc(sizeof(struct dxStrObj));
...
CCLAPI
RCODE dxStrCatB(DxStr _this, const char* s,unsigned int len)
{
	long newLen;
	
	if ( _this==NULL || s==NULL ) return RC_ERROR;;
	
	newLen = _this->len_+len;
	if( newLen >= _this->alloced_ ) {
		char*  tmp = NULL;
		long   org_alloced = _this->alloced_;
	...
	memcpy((_this->text_) + (_this->len_), s ,len);
	_this->len_ = newLen;
	return RC_OK;
}
