 ...
 * sdp_tm.c
 *
 * $Id: sdp_tm.c,v 1.4 2008/12/05 09:09:00 tyhuang Exp $
 */
...
mediaType_success:
	...
	*/
		while( *MEDIA_PROTOCOL_TOKEN[i]!='\0' ) {
			...
			if (*tokListIter->pToken==MEDIA_PROTOCOL_TOKEN[i][0]) {
				if( strcmp(MEDIA_PROTOCOL_TOKEN[i],tokListIter->pToken)==0 ) {
					media->protocol_  = i;
					memset(media->protocol_name_,0,SDP_MAX_TM_TOKEN_SIZE);
					strcpy(media->protocol_name_,tokListIter->pToken);
		...
		if((tokListIter->pToken!=NULL)
			&&(strlen(tokListIter->pToken)>0)
			&&(strlen(tokListIter->pToken)<SDP_MAX_TM_TOKEN_SIZE))
		{
			memset(media->protocol_name_,0,SDP_MAX_TM_TOKEN_SIZE);
			media->protocol_  = i;
			strcpy(media->protocol_name_,tokListIter->pToken);
