 ...
 * sdp_tses.c
 *
 * $Id: sdp_tses.c,v 1.1.1.1 2008/08/01 07:00:24 judy Exp $
 */
...
{
	SdpTsess	timeSess = (SdpTsess)calloc(1,sizeof(struct sdpTsessObj));
...
SdpTr* sdpTrDup(SdpTr* reptime)
{
	...
	dup = (SdpTr*)calloc(1,sizeof(SdpTr));
	if(NULL==dup)
		return NULL;
...
SdpTsess sdpTsessDup(SdpTsess timeSess)
{
	...
	dup = (SdpTsess)calloc(1,sizeof(struct sdpTsessObj));
	if(NULL==dup)
		return NULL;
...
RCODE	sdpTsessAddTrAt(SdpTsess timeSess, SdpTr* reptime, int index)
{
	...
	temp = (SdpTr*)calloc(1,sizeof(SdpTr));
	if(NULL==temp)
		return RC_ERROR;
