				 ...
				 sdp_session_t const *sdp)
{
  ...
  //==>BILL : avoid version is too big to communcate with itri(short)
  if (o->o_id == 0)
  {  
      su_randmem(&o->o_id, sizeof(unsigned int));
  }
  o->o_id &= ((unsigned int)1 << 31) - 1;
    
  if (o->o_version == 0)
  {
      su_randmem(&o->o_version, sizeof(unsigned int));
  }
  o->o_version &= ((unsigned int)1 << 31) - 1;
  //<==BILL : avoid version is too big to communcate with itri(short)
