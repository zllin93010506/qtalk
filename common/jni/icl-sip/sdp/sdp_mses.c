 ...
 * sdp_mses.c
 *
 * $Id: sdp_mses.c,v 1.1.1.1 2008/08/01 07:00:24 judy Exp $
 */
...
SdpTa*	sdpTaDup(SdpTa*	attrib)
{
	...
	temp = (SdpTa*)calloc(1,sizeof(SdpTa));
...
SdpTc*	sdpTcDup(SdpTc*	conndata)
{
	...
	temp = (SdpTc*)calloc(1,sizeof(SdpTc));
...
*/
	if( mediaSess->pTb_ ) {
		...
		sdp->pTb_ = calloc(1,sizeof(SdpTb));
		if(sdp->pTb_!=NULL)
			(*sdp->pTb_) = (*mediaSess->pTb_);
	}
	if( mediaSess->pTk_ ) {
		/* sdp->pTk_ = (SdpTk*)malloc(sizeof(SdpTk)); */
		sdp->pTk_ = (SdpTk*)calloc(1,sizeof(SdpTk));
...
RCODE		sdpMsessSetTb(SdpMsess mediaSess, SdpTb* bandwidth)
{
	...
	if(!mediaSess->pTb_) 
		mediaSess->pTb_ = (SdpTb*)calloc(1,sizeof(SdpTb));
...
RCODE		sdpMsessSetTk(SdpMsess mediaSess, SdpTk* encrykey)
{
	...
	if(!mediaSess->pTk_) 
		mediaSess->pTk_ = (SdpTk*)calloc(1,sizeof(SdpTk));
