 ...
 * sip_req.c
 *
 * $Id: sip_req.c,v 1.13 2009/02/06 07:45:11 tyhuang Exp $
 */
...
struct sipReqObj {
	...
	char			*reqbuf;
	unsigned int	reqbuflen;

	MsgHdr			hdrlist;
	void*			tx;

	BOOL			UseShortForm; /* add by tyhuang 2008/9/12 */
	...
	unsigned char	*accept_contact;
	unsigned char	*history_info;

	unsigned char	*paccessnetworkinfo; 
	unsigned char	*passertedservice;
	unsigned char	*ppreferredservice;
	unsigned char	*pdebugid;
	unsigned char	*pprofilekey;
	unsigned char	*pserveduser;
	unsigned char	*pprivatenetworkindication;
	unsigned char	*pearlymedia;
	unsigned char	*reason;
	unsigned char	*resourcepriority;
...
SipReq sipReqNew(void)
{
	...
	_this->UseShortForm = g_sipUseShortForm;
...
SipReq sipReqDup(SipReq _source)
{
	...
	if(_source){
		...
		if(_source->refersub)
			ReferSubReqDup(_this,_source->refersub);
		...
		_this->UseShortForm=_source->UseShortForm;
	...
	*/	
		...
		if(_source->pchargingvector)
			PCVReqDup(_this,_source->pchargingvector);
		...
		if(_source->accept_contact)
			AcceptContactReqDup(_this,_source->accept_contact);
		
		if(_source->history_info)
			HistoryInfoReqDup(_this,_source->history_info);

		if(_source->paccessnetworkinfo)
			PANIReqDup(_this,_source->paccessnetworkinfo);

		if(_source->passertedservice)
			PASReqDup(_this,_source->passertedservice);

		if(_source->ppreferredservice)
			PPSReqDup(_this,_source->ppreferredservice);

		if(_source->pdebugid)
			PDIDReqDup(_this,_source->pdebugid);

		if(_source->pprofilekey)
			PPKReqDup(_this,_source->pprofilekey);

		if(_source->pserveduser)
			PSUReqDup(_this,_source->pserveduser);

		if(_source->pprivatenetworkindication)
			PPNIReqDup(_this,_source->pprivatenetworkindication);

		if(_source->pearlymedia)
			PEMReqDup(_this,_source->pearlymedia);

		if(_source->reason)
			ReasonReqDup(_this,_source->reason);

		if(_source->resourcepriority)
			RPReqDup(_this,_source->resourcepriority);

		if(_source->sessionid)
			SessionIDReqDup(_this,_source->sessionid);
...
void sipReqFree(SipReq request)
{
	...
	if(request->pchargingvector) free(request->pchargingvector);
	...
	if(request->accept_contact) free(request->accept_contact);
	if(request->history_info)	free(request->history_info);
	if(request->paccessnetworkinfo) free(request->paccessnetworkinfo);
	if(request->passertedservice) free(request->passertedservice);
	if(request->ppreferredservice) free(request->ppreferredservice);
	if(request->pdebugid) free(request->pdebugid);
	if(request->pprofilekey) free(request->pprofilekey);
	if(request->pserveduser) free(request->pserveduser);
	if(request->pprivatenetworkindication) free(request->pprivatenetworkindication);
	if(request->pearlymedia) free(request->pearlymedia);
	if(request->reason) free(request->reason);
	if(request->resourcepriority) free(request->resourcepriority);
	if(request->sessionid) free(request->sessionid);
...
RCODE sipReqAddHdr(SipReq request, SipHdrType headertype, void* str)
{
	...
	*/
	...
	case In_Reply_To:
		...
		break;
	case Join:
		JoinReqDup(request,(SipJoin*)str);
		break;
	case Max_Forwards:
	...
	case Record_Route:
		...
		break;
	case Refer_Sub:
		ReferSubReqDup(request,(SipReferSub*)str);
		break;
	case Refer_To:
	...
	case P_Charging_Vector:
		PCVReqDup(request,(unsigned char*)str);
		break;
...
#endif /* end of ICL_IMS */	
	case Accept_Contact:
		AcceptContactReqDup(request,(unsigned char *)str);
		break;
	case History_Info:
		HistoryInfoReqDup(request,(unsigned char *)str);
		break;
	case P_Access_Network_Info:
		PANIReqDup(request,(unsigned char *)str);
		break;
	case P_Asserted_Service:
		PASReqDup(request,(unsigned char *)str);
		break;
	case P_Preferred_Service:
		PPSReqDup(request,(unsigned char *)str);
		break;
	case P_Debug_ID:
		PDIDReqDup(request,(unsigned char *)str);
		break;
	case P_Profile_Key:
		PPKReqDup(request,(unsigned char *)str);
		break;
	case P_Served_User:
		PSUReqDup(request,(unsigned char *)str);
		break;
	case P_Private_Network_Indication:
		PPNIReqDup(request,(unsigned char *)str);
		break;
	case P_Early_Media:
		PEMReqDup(request,(unsigned char *)str);
		break;
	case Reason:
		ReasonReqDup(request,(unsigned char *)str);
		break;
	case Resource_Priority:
		RPReqDup(request,(unsigned char *)str);
		break;
	case Session_ID:
		SessionIDReqDup(request,(unsigned char *)str);
		break;
	default:
   ...
   switch(headertype){
		...
		*/
		...
		case Record_Route:
			...
			break;
		case Refer_Sub:
			if(request->refersub != NULL){
				ReferSubFree((SipReferSub*)request->refersub);
			}
			request->refersub=(SipReferSub*)ret;
			break;
		case Refer_To:
		case Refer_To_Short:
		...
		case P_Charging_Vector:
			...
			request->pchargingvector=(unsigned char*)ret;
			break;
		case P_Visited_Network_ID:
		...
		case Accept_Contact_Short:
			if(request->accept_contact != NULL)  
				free(request->accept_contact);
			request->accept_contact=(unsigned char*)ret;
			break;
		case History_Info:
			if(request->history_info != NULL)  
				free(request->history_info);
			request->history_info=(unsigned char*)ret;
			break;
		case P_Access_Network_Info:
			...
			break;
		case P_Asserted_Service:
			if(request->passertedservice != NULL)  
				free(request->passertedservice);
			request->passertedservice=(unsigned char*)ret;
			break;
		case P_Preferred_Service:
			if(request->ppreferredservice != NULL)  
				free(request->ppreferredservice);
			request->ppreferredservice=(unsigned char*)ret;
			break;
		case P_Debug_ID:
			if(request->pdebugid != NULL)  
				free(request->pdebugid);
			request->pdebugid=(unsigned char*)ret;
			break;
		case P_Profile_Key:
			if(request->pprofilekey != NULL)  
				free(request->pprofilekey);
			request->pprofilekey=(unsigned char*)ret;
			break;
		case P_Served_User:
			if(request->pserveduser != NULL)  
				free(request->pserveduser);
			request->pserveduser=(unsigned char*)ret;
			break;
		case P_Private_Network_Indication:
			if(request->pprivatenetworkindication != NULL)  
				free(request->pprivatenetworkindication);
			request->pprivatenetworkindication=(unsigned char*)ret;
			break;
		case P_Early_Media:
			if(request->pearlymedia != NULL)  
				free(request->pearlymedia);
			request->pearlymedia=(unsigned char*)ret;
			break;
		case Reason:
			if(request->reason != NULL)  
				free(request->reason);
			request->reason=(unsigned char*)ret;
			break;
		case Resource_Priority:
			if(request->resourcepriority != NULL)  
				free(request->resourcepriority);
			request->resourcepriority=(unsigned char*)ret;
			break;
		case Session_ID:
			if(request->sessionid != NULL)  
				free(request->sessionid);
			request->sessionid=(unsigned char*)ret;
			break;
		case UnknSipHdr:
			break;
		default:
...
RCODE sipReqAddHdrFromTxt(SipReq request, SipHdrType type, char* header)
{
	...
	request->reqflag=TRUE;
	tmpbuf=(char*)calloc(hdrlen+2,1);
	strcpy(tmpbuf,header);
	hdr=msgHdrNew();
	rfc822_parseline((unsigned char*)tmpbuf,hdrlen,hdr);
	tmp=hdr;
...
*/
		...
		case P_Charging_Vector:
			return (void *)request->pchargingvector;
		case P_Visited_Network_ID:
		...
		case Accept_Contact_Short:
		case Accept_Contact:
			return (void *)request->accept_contact;
		case History_Info:
			return (void *)request->history_info;
		case P_Access_Network_Info:
			return (void *)request->paccessnetworkinfo;
		case P_Asserted_Service:
			return (void *)request->passertedservice;
		case P_Preferred_Service:
			return (void *)request->ppreferredservice;
		case P_Debug_ID:
			return (void *)request->pdebugid;
		case P_Profile_Key:
			return (void *)request->pprofilekey;
		case P_Served_User:
			return (void *)request->pserveduser;
		case P_Private_Network_Indication:
			return (void *)request->pprivatenetworkindication;
		case P_Early_Media:
			return (void *)request->pearlymedia;
		case Reason:
			return (void *)request->reason;
		case Resource_Priority:
			return (void *)request->resourcepriority;
		case Session_ID:
			return (void *)request->sessionid;
		default:
			return (void *)NULL;
	}
}
...
/* Set use short form or not */
CCLAPI 
RCODE sipReqSetUseShortForm(IN SipReq request,IN BOOL useshortform)
{
	if(request){
		request->UseShortForm=useshortform;
		return RC_OK;
	}else
		return RC_ERROR;
...
RCODE sipReqSetBody(SipReq request,SipBody *content)
{
	...
	if((request == NULL)||(NULL==content))
	   return RC_ERROR;

	request->reqflag=TRUE;
	_body = (SipBody*)calloc(1,sizeof(*_body));
	/* _body->content = (unsigned char*)strDup((const char*)content->content); */
	if(!_body) return RC_ERROR;
	else
		_body->content=calloc(1,content->length+1);
	memcpy(_body->content,content->content,content->length);
...
CCLAPI 
RCODE sipReqRemoveBody(IN SipReq request)
{
	if(NULL==request)
		return RC_ERROR;

	/* set modified flag on */
	request->reqflag=TRUE;
...
{
	return sipReqNewFromTxtEx(text,0);
}

CCLAPI 
SipReq sipReqNewFromTxtEx(IN char* bin,IN unsigned int len)
{
	SipReq	request=NULL;
	char*		temp;
	int		length=len;
...
char*	sipReqPrint(SipReq request)
{
	...
	if(request->reqflag==TRUE){
		...
		request->reqbuflen=dxStrLen(ReqBuf);
		request->reqbuf=calloc(1,request->reqbuflen+1);
		memcpy(request->reqbuf,dxStrAsCStr(ReqBuf),request->reqbuflen);
		dxStrFree(ReqBuf);
		request->reqflag=TRUE;
	}
...
CCLAPI 
unsigned int sipReqLen(IN SipReq request)
{
	if(NULL==request)
		return 0;
	else
		return request->reqbuflen;
}
...
int AcceptReqDup(SipReq request,SipAccept *accept )
{
	...
	*/
	request->accept=AcceptDup(accept);
	return 1;
}
...
int AcceptEncodingReqDup(SipReq request,SipAcceptEncoding* coding)
{
	...
	*/
	request->acceptencoding=AcceptEncodingDup(coding);
	return 1;
}
...
int AcceptLanguageReqDup(SipReq request, SipAcceptLang* lang)
{
	...
	*/
	request->acceptlanguage=AcceptLanguageDup(lang);
	return 1;
}
...
int AllowEventsReqDup(SipReq request, SipAllowEvt *allowevt)
{
	...
	request->allowevents=AllowEventsDup(allowevt);
	return 1;
}
...
int AlertInfoReqDup(SipReq request,SipInfo *Alertinfo)
{
	...
	request->alertinfo=SipInfoDup(Alertinfo);
	/*
	tmpAlertinfo=NULL;
	while(Alertinfo!=NULL){
		...
		if(Alertinfo->SipParamList!=NULL)
			nowAlertinfo->SipParamList=sipParameterDupAll(Alertinfo->SipParamList);
...
int CallInfoReqDup(SipReq request,SipInfo *Callinfo)
{
	...
	request->callinfo=SipInfoDup(Callinfo);
...
int ContentDispositionReqDup(SipReq request,SipContentDisposition* contentdisp)
{
	...
	if(contentdisp->paramList != NULL)
		request->contentdisposition->paramList=sipParameterDupAll(contentdisp->paramList);
	return 1;
}
...
int ContentEncodingReqDup(SipReq request,SipContEncoding* contcoding)
{
	...
	*/
	request->contentencoding=ContentEncodingDup(contcoding);
	return 1;
}
...
int ContentTypeReqDup(SipReq request,SipContType* conttype)
{
	...
	*/
	request->contenttype=ContentTypeDup(conttype);
	return 1;
}
...
int ExpiresReqDup(SipReq request,SipExpires* expire)
{
	...
	request->expires=ExpiresDup(expire);
...
int JoinReqDup(SipReq request,SipJoin *join)
{
	SipJoin *reqjoin;
	if(request->join!=NULL)
		JoinFree(request->join);

	request->join=(SipJoin*)calloc(1,sizeof(SipJoin));
	if(NULL==request->join)
		return 0;
	
	reqjoin=request->join;
	if(join!=NULL){
		reqjoin->callid=strDup(join->callid);
		reqjoin->fromtag=strDup(join->fromtag);
		reqjoin->totag=strDup(join->totag);

		reqjoin->sipParamList=sipParameterDupAll(join->sipParamList);
	}
...
int PrivacyReqDup(SipReq request,SipPrivacy *pPrivacy)
{
	...
	request->privacy=PrivacyDup(pPrivacy);
...
int ReferSubReqDup(SipReq req,SipReferSub* refersub)
{

	if(req==NULL)
		return 0;

	if(req->refersub != NULL)
		ReferSubFree(req->refersub);

	req->refersub=ReferSubDup(refersub);
...
int ReferToReqDup(SipReq req,SipReferTo* referto)
{
	...
	req->referto = ReferToDup(referto);
...
int ReferredByReqDup(SipReq req,SipReferredBy* referredby)
{
	...
	req->referredby = ReferredByDup(referredby);
...
int ReplyToReqDup(SipReq request,SipReplyTo* _replyto)
{
	...
	request->replyto = ReplyToDup(_replyto);
...
int RequireReqDup(SipReq request,SipRequire* require)
{
	...
	request->require=RequireDup(require);
...
int SessionExpiresReqDup(SipReq req,SipSessionExpires *_sessionexpires)
{
	...
	if(req->sessionexpires != NULL){
		...
		req->sessionexpires=NULL;
	}

	req->sessionexpires=SessionExpiresDup(_sessionexpires);
...
int SubscriptionStateReqDup(SipReq request, SipSubState *substate)
{
	...
	if(substate != NULL){
		...
		request->subscriptionstate->paramList=sipParameterDupAll(substate->paramList);
	}
	return 1;
}
...
int SupportedReqDup(SipReq request,SipSupported *support)
{
	...
	request->supported=SupportedDup(support);
...
int UserAgentReqDup(SipReq request,SipUserAgent* agent)
{
	...
	request->useragent=UserAgentDup(agent);
...
/* Accept-Contact */
int	AcceptContactReqDup(SipReq request,const unsigned char* str)
{
	if(request->accept_contact != NULL)
		free(request->accept_contact);
	request->accept_contact=(unsigned char*)strDup((char*)str);
	return 1;
}

/* History-Info */
int	HistoryInfoReqDup(SipReq request,const unsigned char* str)
{
	if(request->history_info != NULL)
		free(request->history_info);
	request->history_info=(unsigned char*)strDup((char*)str);
	return 1;
}

/* P-Access-Network-Info */
int	PANIReqDup(SipReq request,const unsigned char* str)
{
	if(request->paccessnetworkinfo != NULL)
		free(request->paccessnetworkinfo);
	request->paccessnetworkinfo=strDup(str);
	return 1;
}

/* P-Asserted-Service */
int	PASReqDup(SipReq request,const unsigned char* str)
{
	if(request->passertedservice != NULL)
		free(request->passertedservice);
	request->passertedservice=(unsigned char*)strDup((char*)str);
	return 1;
}

/* P-Preferred-Service */
int	PPSReqDup(SipReq request,const unsigned char* str)
{
	if(request->ppreferredservice != NULL)
		free(request->ppreferredservice);
	request->ppreferredservice=(unsigned char*)strDup((char*)str);
	return 1;
}

/* P-Debug-ID */
int	PDIDReqDup(SipReq request,const unsigned char* str)
{
	if(request->pdebugid != NULL)
		free(request->pdebugid);
	request->pdebugid=(unsigned char*)strDup((char*)str);
	return 1;
}

/* P-Profile-Key */
int	PPKReqDup(SipReq request,const unsigned char* str)
{
	if(request->pprofilekey != NULL)
		free(request->pprofilekey);
	request->pprofilekey=(unsigned char*)strDup((char*)str);
	return 1;
}

/* P-Served-User */
int	PSUReqDup(SipReq request,const unsigned char* str)
{
	if(request->pserveduser != NULL)
		free(request->pserveduser);
	request->pserveduser=(unsigned char*)strDup((char*)str);
	return 1;
}

/* P-Private-Network-Indication */
int	PPNIReqDup(SipReq request,const unsigned char* str)
{
	if(request->pprivatenetworkindication != NULL)
		free(request->pprivatenetworkindication);
	request->pprivatenetworkindication=(unsigned char*)strDup((char*)str);
	return 1;
}

/* P-Early-Media */
int	PEMReqDup(SipReq request,const unsigned char* str)
{
	if(request->pearlymedia != NULL)
		free(request->pearlymedia);
	request->pearlymedia=(unsigned char*)strDup((char*)str);
	return 1;
}

/* Reason */
int	ReasonReqDup(SipReq request,const unsigned char* str)
{
	if(request->reason != NULL)
		free(request->reason);
	request->reason=(unsigned char*)strDup((char*)str);
	return 1;
}

/* Resource-Priority */
int	RPReqDup(SipReq request,const unsigned char* str)
{
	if(request->resourcepriority != NULL)
		free(request->resourcepriority);
	request->resourcepriority=(unsigned char*)strDup((char*)str);
	return 1;
}

/* Session-ID */
int	SessionIDReqDup(SipReq request,const unsigned char* str)
{
	if(request->sessionid != NULL)
		free(request->sessionid);
	request->sessionid=(unsigned char*)strDup((char*)str);
	return 1;
}
...
int sipReqParse(SipReq request, unsigned char* msg, int* length)
{
  ...
  /*if(bcorrect)*/
	if((request->contentlength > 0)&&(hdrlength>0)){
		int len=*length-hdrlength;
		SipBody* _pby;

		if(len>0){
			_pby = (SipBody* )calloc(1,sizeof(*_pby));
			if(_pby){
				request->reqflag=TRUE;
				_pby->length=request->contentlength;
				_pby->content=(unsigned char *)calloc(1,_pby->length+1);
				if(_pby->content){
					memcpy(_pby->content,msg+hdrlength,_pby->length);
					request->body=_pby;
				}else
					free(_pby);
			}
...
int sipReqAsStr(SipReq request, DxStr ReqBuf, int *length)
{
  ...
  *length=dxStrLen(RequestStr);
  ...
  if(request->allowevents != NULL){
	  AllowEventsAsStr(request->allowevents,HeaderBuf,Len,&tmp,request->UseShortForm);
	  dxStrCat(RequestStr,(char*)HeaderBuf);
	  lenstr+=tmp;
  }
  if (request->authorization != NULL) {
  ...
  if (request->callid != NULL) {
	CallIDAsStr(request->callid, HeaderBuf, Len, &tmp,request->UseShortForm);
	dxStrCat(RequestStr,(char*)HeaderBuf);
	lenstr = lenstr+tmp;
  }
  if(request->callinfo != NULL){
  ...
  */
  if (request->contact != NULL) {
	sipContactAsStr(request->contact, HeaderBuf, Len, &tmp,request->UseShortForm);
	dxStrCat(RequestStr,(char*)HeaderBuf);
	lenstr = lenstr+tmp;
  }
  if (request->contentdisposition!= NULL) {
  ...
  if (request->contentencoding != NULL) {
	ContentEncodingAsStr(request->contentencoding, HeaderBuf, Len, &tmp,request->UseShortForm);
	dxStrCat(RequestStr,(char*)HeaderBuf);
	lenstr = lenstr+tmp;
  }
  if (request->contentlanguage != NULL) {
  ...
  if (request->contentlength != -1) {
	ContentLengthAsStr(request->contentlength,HeaderBuf, Len, &tmp,request->UseShortForm);
	dxStrCat(RequestStr,(char*)HeaderBuf);
	lenstr = lenstr+tmp;
  }
  if (request->contenttype != NULL) {
	ContentTypeAsStr(request->contenttype,HeaderBuf, Len, &tmp,request->UseShortForm);
	dxStrCat(RequestStr,(char*)HeaderBuf);
	lenstr = lenstr+tmp;
  }
  if (request->cseq != NULL) {
  ...
  if (request->event !=NULL){
	  EventAsStr(request->event,HeaderBuf,Len,&tmp,request->UseShortForm);
	  dxStrCat(RequestStr,(char*)HeaderBuf);
	  lenstr=lenstr+tmp;
  }
  if (request->expires != NULL) {
  ...
  if (request->from != NULL) {
	FromAsStr(request->from,HeaderBuf, Len, &tmp,request->UseShortForm);
	dxStrCat(RequestStr,(char*)HeaderBuf);
	lenstr = lenstr+tmp;
  }
  ...
  if (request->join != NULL) {
	JoinAsStr(request->join,HeaderBuf, Len, &tmp);
	dxStrCat(RequestStr,(char*)HeaderBuf);
	lenstr = lenstr+tmp;
  }
  ...
  if(request->refersub != NULL){
	  ReferSubAsStr(request->refersub,HeaderBuf,Len, &tmp);
	  dxStrCat(RequestStr,(char*)HeaderBuf);
	  lenstr=lenstr+tmp;
  }
  if(request->referto != NULL){
	  ReferToAsStr(request->referto,HeaderBuf,Len, &tmp,request->UseShortForm);
	  dxStrCat(RequestStr,(char*)HeaderBuf);
	  lenstr=lenstr+tmp;
  }
  if(request->referredby != NULL){
	  ReferredByAsStr(request->referredby,HeaderBuf,Len, &tmp,request->UseShortForm);
	  dxStrCat(RequestStr,(char*)HeaderBuf);
	  lenstr=lenstr+tmp;
  }
  if(request->remotepartyid != NULL){
  ...
  if(request->sessionexpires != NULL){
	  SessionExpiresAsStr(request->sessionexpires,HeaderBuf,Len,&tmp,request->UseShortForm);
	  dxStrCat(RequestStr,(char*)HeaderBuf);
	  lenstr=lenstr+tmp;
  }
  if(request->sipifmatch != NULL){
  ...
  if (request->subject != NULL) {
	SubjectAsStr(request->subject,HeaderBuf, Len, &tmp,request->UseShortForm);
	dxStrCat(RequestStr,(char*)HeaderBuf);
	lenstr = lenstr+tmp;
  }
  if (request->subscriptionstate != NULL){
  ...
  if (request->supported != NULL){
	  SupportedAsStr(request->supported,HeaderBuf,Len,&tmp,request->UseShortForm);
	  dxStrCat(RequestStr,(char*)HeaderBuf);
	  lenstr+=tmp;
  }
  if (request->timestamp != NULL) {
  ...
  if (request->to != NULL) {
	ToAsStr(request->to, HeaderBuf, Len, &tmp,request->UseShortForm);
	dxStrCat(RequestStr,(char*)HeaderBuf);
	lenstr = lenstr+tmp;
  }
  if (request->useragent != NULL) {
  ...
  if (request->via != NULL) {
	ViaAsStr(request->via,HeaderBuf, Len, &tmp,request->UseShortForm);
	dxStrCat(RequestStr,(char*)HeaderBuf);
	lenstr = lenstr+tmp;
  }
  ...
  if (request->pchargingvector != NULL) {
	  if(PCVAsStr(request->pchargingvector,HeaderBuf, Len, &tmp)!=0){
		  dxStrCat(RequestStr,(char*)HeaderBuf);
		  lenstr = lenstr+tmp;
	  }
  }
  ...
  /* from quantatw.com */
  if (request->accept_contact != NULL) {
	  if(AcceptContactAsStr(request->accept_contact,HeaderBuf, Len, &tmp)!=0){
		dxStrCat(RequestStr,(char*)HeaderBuf);
		lenstr = lenstr+tmp;
	  }
  }
  ...
  if (request->history_info != NULL) {
	  if(HistoryInfoAsStr(request->history_info,HeaderBuf, Len, &tmp)!=0){
		dxStrCat(RequestStr,(char*)HeaderBuf);
		lenstr = lenstr+tmp;
	  }
  }
  ...
  if (request->passertedservice != NULL) {
	  if(PASAsStr(request->passertedservice,HeaderBuf, Len, &tmp)!=0){
		dxStrCat(RequestStr,(char*)HeaderBuf);
		lenstr = lenstr+tmp;
	  }
  }
  ...
  if (request->ppreferredservice != NULL) {
	  if(PPSAsStr(request->ppreferredservice,HeaderBuf, Len, &tmp)!=0){
		dxStrCat(RequestStr,(char*)HeaderBuf);
		lenstr = lenstr+tmp;
	  }
  }
  ...
  if (request->pdebugid != NULL) {
	  if(PDIDAsStr(request->pdebugid,HeaderBuf, Len, &tmp)!=0){
		dxStrCat(RequestStr,(char*)HeaderBuf);
		lenstr = lenstr+tmp;
	  }
  }
  if (request->pprofilekey != NULL) {
	  if(PPKAsStr(request->pprofilekey,HeaderBuf, Len, &tmp)!=0){
		dxStrCat(RequestStr,(char*)HeaderBuf);
		lenstr = lenstr+tmp;
	  }
  }
  if (request->pserveduser != NULL) {
	  if(PSUAsStr(request->pserveduser,HeaderBuf, Len, &tmp)!=0){
		dxStrCat(RequestStr,(char*)HeaderBuf);
		lenstr = lenstr+tmp;
	  }
  }
  if (request->pprivatenetworkindication != NULL) {
	  if(PPNIAsStr(request->pprivatenetworkindication,HeaderBuf, Len, &tmp)!=0){
		dxStrCat(RequestStr,(char*)HeaderBuf);
		lenstr = lenstr+tmp;
	  }
  }
  if (request->pearlymedia != NULL) {
	  if(PEMAsStr(request->pearlymedia,HeaderBuf, Len, &tmp)!=0){
		dxStrCat(RequestStr,(char*)HeaderBuf);
		lenstr = lenstr+tmp;
	  }
  }
  if (request->reason != NULL) {
	  if(ReasonAsStr(request->reason,HeaderBuf, Len, &tmp)!=0){
		dxStrCat(RequestStr,(char*)HeaderBuf);
		lenstr = lenstr+tmp;
	  }
  }
  if (request->resourcepriority != NULL) {
	  if(RPAsStr(request->resourcepriority,HeaderBuf, Len, &tmp)!=0){
		dxStrCat(RequestStr,(char*)HeaderBuf);
		lenstr = lenstr+tmp;
	  }
  }
  if (request->sessionid != NULL) {
	  if(SessionIDAsStr(request->sessionid,HeaderBuf, Len, &tmp)!=0){
		dxStrCat(RequestStr,(char*)HeaderBuf);
		lenstr = lenstr+tmp;
	  }
  }
  ...
  if(request->body != NULL) {
	  if(request->body->length > 0){
		  ...
		  dxStrCatB(RequestStr,(char*)request->body->content,request->contentlength);
	  }
  }
	...
	)
{
	...
	/* A1 */
	if( strCmp(auth->algorithm_,"MD5-sess")==0 ) {
		temp[0] = 0;	A1[0] = 0;
	...
	/* A2 */
	if(strCmp(auth->message_qop_,"auth-int")==0 ) {
		if( !entity_body )
			return RC_ERROR;
		A2[0] = 0;
