 ...
 * sip_rsp.c
 *
 * $Id: sip_rsp.c,v 1.10 2009/02/06 07:45:11 tyhuang Exp $
 */
...
SipRsp sipRspDup(SipRsp _source)
{
	...
	if(_source){
		...
		if(_source->refersub)
			ReferSubRspDup(_this,_source->refersub);
		...
		if(_source->server)
			ServerHdrRspDup(_this,_source->server);
		...
		if(_source->history_info)
			HistoryInfoRspDup(_this,_source->history_info);

		if(_source->paccessnetworkinfo)
			PANIRspDup(_this,_source->paccessnetworkinfo);

		if(_source->pearlymedia)
			PEMRspDup(_this,_source->pearlymedia);

		if(_source->reason)
			ReasonRspDup(_this,_source->reason);

		if(_source->sessionid)
			SessionIDRspDup(_this,_source->sessionid);
...
void sipRspFree(SipRsp response)
{
	...
	ReferSubFree(response->refersub);
	...
	if(response->history_info) free(response->history_info);
	if(response->paccessnetworkinfo) free(response->paccessnetworkinfo);
	if(response->pearlymedia) free(response->pearlymedia);
	if(response->reason) free(response->reason);
	if(response->sessionid) free(response->sessionid);
...
#endif /* end of ICL_IMS */
	case History_Info:	
		return (void *)response->history_info;
	case P_Access_Network_Info:	
		return (void *)response->paccessnetworkinfo;
	case P_Early_Media:	
		return (void *)response->pearlymedia;
	case Reason:	
		return (void *)response->reason;
	case Session_ID:	
		return (void *)response->sessionid;
	default:
		return (void *)NULL;
  }
}
...
RCODE sipRspAddHdr(SipRsp response, SipHdrType headertype, void *data)
{
	...
	if(data != NULL){
		...
		case Record_Route:
			...
			break;
		case Refer_Sub:
			ReferSubRspDup(response,(SipReferSub*)data);
			break;
		case Refer_To:
		case Refer_To_Short:
		...
		case Session_Expires_Short:
			...
			break;
		case Server:
			ServerHdrRspDup(response,(SipServer*)data);
			break;
		case SIP_ETag:
...
#endif /* end of ICL_IMS */
		case History_Info:	
			HistoryInfoRspDup(response,(unsigned char*)data);
			break;
		case P_Access_Network_Info:	
			PANIRspDup(response,(unsigned char*)data);
			break;
		case P_Early_Media:	
			PEMRspDup(response,(unsigned char*)data);
			break;
		case Reason:	
			ReasonRspDup(response,(unsigned char*)data);
			break;
		case Session_ID:	
			SessionIDRspDup(response,(unsigned char*)data);
			break;
		default:
...
RCODE sipRspAddHdrEx(SipRsp response, SipHdrType headertype, void *ret)
{
	...
	if(ret != NULL)
		...
		case Record_Route:
			...
			break;
		case Refer_Sub:
			if(response->refersub != NULL){
				ReferSubFree((SipReferSub*)response->refersub);
			}
			response->refersub=(SipReferSub*)ret;
			break;
		case Refer_To:
		case Refer_To_Short:
		...
		case History_Info:	
			if(response->history_info != NULL)
				free(response->history_info);
			response->history_info=(char*)ret;
			break;
		case P_Access_Network_Info:	
			if(response->paccessnetworkinfo != NULL)
				free(response->paccessnetworkinfo);
			response->paccessnetworkinfo=(char*)ret;
			break;
		case P_Early_Media:	
			if(response->pearlymedia != NULL)
				free(response->pearlymedia);
			response->pearlymedia=(char*)ret;
			break;
		case Reason:	
			if(response->reason != NULL)
				free(response->reason);
			response->reason=(char*)ret;
			break;
		case Session_ID:	
			if(response->sessionid != NULL)
				free(response->sessionid);
			response->sessionid=(char*)ret;
			break;
		default:
...
CCLAPI 
RCODE sipRspSetUseShortForm(IN SipRsp response,IN BOOL useshortform)
{
	if(NULL!=response){
		response->UseShortForm=useshortform;
		return RC_OK;
	}else
		return RC_ERROR;
...
int sipRspAsStr(SipRsp response, DxStr RspBuf, int *length)
{
	...
	*length=dxStrLen(ResponseStr);
	...
	if(response->allowevents != NULL){
		AllowEventsAsStr(response->allowevents,HeaderBuf, Len, &tmp,response->UseShortForm);
		dxStrCat(ResponseStr,(char*)HeaderBuf);
		lenstr = lenstr+tmp;
	}
	if(response->authenticationinfo != NULL) {
	...
	if (response->callid != NULL) {
		CallIDAsStr(response->callid,HeaderBuf, Len, &tmp,response->UseShortForm);
		dxStrCat(ResponseStr,(char*)HeaderBuf);
		lenstr = lenstr+tmp;
	}
	if(response->callinfo != NULL) {
	...
	if (response->contact != NULL) {
		sipContactAsStr(response->contact,HeaderBuf, Len, &tmp,response->UseShortForm);
		dxStrCat(ResponseStr,(char*)HeaderBuf);
		lenstr = lenstr+tmp;
	}
	if (response->contentdisposition != NULL) {
	...
	if (response->contentencoding != NULL) {
		ContentEncodingAsStr(response->contentencoding,HeaderBuf, Len, &tmp,response->UseShortForm);
		dxStrCat(ResponseStr,(char*)HeaderBuf);
		lenstr = lenstr+tmp;
	}
	if (response->contentlanguage != NULL) {
	...
	if (response->contentlength != -1) {
		ContentLengthAsStr(response->contentlength,HeaderBuf, Len, &tmp,response->UseShortForm);
		dxStrCat(ResponseStr,(char*)HeaderBuf);
		lenstr = lenstr+tmp;
	}
	if (response->contenttype != NULL) {
		ContentTypeAsStr(response->contenttype,HeaderBuf, Len, &tmp,response->UseShortForm);
		dxStrCat(ResponseStr,(char*)HeaderBuf);
		lenstr = lenstr+tmp;
	}
	if (response->cseq != NULL) {
	...
	if (response->event != NULL){
		EventAsStr(response->event,HeaderBuf,Len,&tmp,response->UseShortForm);
		dxStrCat(ResponseStr,(char*)HeaderBuf);
		lenstr = lenstr +tmp;
	}
	if (response->expires != NULL) {
	...
	if (response->from != NULL) {
		FromAsStr(response->from, HeaderBuf, Len, &tmp,response->UseShortForm);
		dxStrCat(ResponseStr,(char*)HeaderBuf);
		lenstr = lenstr+tmp;
	}
	if (response->mimeversion != NULL) {
	...
	if(response->refersub != NULL){
		ReferSubAsStr(response->refersub,HeaderBuf,Len, &tmp);
		dxStrCat(ResponseStr,(char*)HeaderBuf);
		lenstr=lenstr+tmp;
	}
	if(response->referto != NULL){
		ReferToAsStr(response->referto,HeaderBuf,Len, &tmp,response->UseShortForm);
		dxStrCat(ResponseStr,(char*)HeaderBuf);
		lenstr=lenstr+tmp;
	}
	if(response->referredby != NULL){
		ReferredByAsStr(response->referredby,HeaderBuf,Len, &tmp,response->UseShortForm);
		dxStrCat(ResponseStr,(char*)HeaderBuf);
		lenstr=lenstr+tmp;
	}
	if(response->remotepartyid != NULL){
	...
	if(response->sessionexpires != NULL){
		SessionExpiresAsStr(response->sessionexpires,HeaderBuf,Len,&tmp,response->UseShortForm);
		dxStrCat(ResponseStr,(char*)HeaderBuf);
		lenstr=lenstr+tmp;
	}
	if (response->server != NULL) {
		ServerHdrAsStr(response->server,HeaderBuf, Len, &tmp);
		dxStrCat(ResponseStr,(char*)HeaderBuf);
		lenstr = lenstr+tmp;
	}
	if (response->sipetag != NULL) {
	...
	if (response->supported != NULL){
		SupportedAsStr(response->supported,HeaderBuf,Len,&tmp,response->UseShortForm);
		dxStrCat(ResponseStr,(char*)HeaderBuf);
		lenstr = lenstr+tmp;
	}
	if (response->timestamp != NULL) {
	...
	if (response->to != NULL) {
		ToAsStr(response->to,HeaderBuf, Len, &tmp,response->UseShortForm);
		dxStrCat(ResponseStr,(char*)HeaderBuf);
		lenstr = lenstr+tmp;
	}
	if (response->unsupported != NULL) {
	...
	if (response->via != NULL) {
		ViaAsStr(response->via,HeaderBuf, Len, &tmp,response->UseShortForm);
		dxStrCat(ResponseStr,(char*)HeaderBuf);
		lenstr = lenstr+tmp;
	}
	if (response->wwwauthenticate != NULL) {
	...
	if (response->history_info != NULL) {
		if(HistoryInfoAsStr(response->history_info,HeaderBuf, Len, &tmp)==1){
			dxStrCat(ResponseStr,(char*)HeaderBuf);
			lenstr = lenstr+tmp;
		}
	}
	...
	if (response->pearlymedia != NULL) {
		if(PEMAsStr(response->pearlymedia,HeaderBuf, Len, &tmp)==1){
			dxStrCat(ResponseStr,(char*)HeaderBuf);
			lenstr = lenstr+tmp;
		}
	}
	if (response->reason != NULL) {
		if(ReasonAsStr(response->reason,HeaderBuf, Len, &tmp)==1){
			dxStrCat(ResponseStr,(char*)HeaderBuf);
			lenstr = lenstr+tmp;
		}
	}
	if (response->sessionid != NULL) {
		if(SessionIDAsStr(response->sessionid,HeaderBuf, Len, &tmp)==1){
			dxStrCat(ResponseStr,(char*)HeaderBuf);
			lenstr = lenstr+tmp;
		}
	}
...
int AcceptRspDup(SipRsp response,SipAccept *accept )
{
	...
	response->accept=AcceptDup(accept);
	return 1;
}
...
int AcceptEncodingRspDup(SipRsp response,SipAcceptEncoding* coding)
{
	...
	response->acceptencoding=AcceptEncodingDup(coding);
	return 1;
}
...
int AcceptLanguageRspDup(SipRsp response, SipAcceptLang* lang)
{
	...
	response->acceptlanguage=AcceptLanguageDup(lang);	
	return 1;
}
...
int AlertInfoRspDup(SipRsp response,SipInfo *Alertinfo)
{
	...
	response->alertinfo=SipInfoDup(Alertinfo);
...
int AllowEventsRspDup(SipRsp response, SipAllowEvt *allowevt)
{
	...
	response->allowevents=AllowEventsDup(allowevt);
	return 1;
}
/**add by tyhuang**************/
...
int CallInfoRspDup(SipRsp response,SipInfo *Callinfo)
{
	...
	response->callinfo=SipInfoDup(Callinfo);
...
int ContentDispositionRspDup(SipRsp response,SipContentDisposition* contentdisp)
{
	...
	if(contentdisp->paramList != NULL)
		response->contentdisposition->paramList=sipParameterDupAll(contentdisp->paramList);
	return 1;
}
...
int ContentEncodingRspDup(SipRsp response,SipContEncoding* contcoding)
{
	...
	response->contentencoding=ContentEncodingDup(contcoding);
	return 1;
}
...
int ContentTypeRspDup(SipRsp response,SipContType* conttype)
{
	...
	response->contenttype=ContentTypeDup(conttype);
	return 1;
}
...
int ErrorInfoRspDup(SipRsp response,SipInfo *Errortinfo)
{
	...
	response->errorinfo=SipInfoDup(Errortinfo);
...
int ExpiresRspDup(SipRsp response,SipExpires* expire)
{
	...
	response->expires=ExpiresDup(expire);
...
int PrivacyRspDup(SipRsp response,SipPrivacy *pPrivacy)
{
	...
	response->privacy=PrivacyDup(pPrivacy);
	return 1;
}
...
int ReferSubRspDup(SipRsp rsp,SipReferSub* refersub)
{

	if(rsp==NULL)
		return 0;

	if(rsp->refersub != NULL)
		ReferSubFree(rsp->refersub);

	rsp->refersub=ReferSubDup(refersub);
...
int ReferToRspDup(SipRsp rsp,SipReferTo* referto)
{
	...
	rsp->referto=ReferToDup(referto);
...
int ReplyToRspDup(SipRsp response,SipReplyTo* _replyto)
{
	...
	response->replyto = ReplyToDup(_replyto);
...
int RequireRspDup(SipRsp response,SipRequire* require)
{
	...
	response->require=RequireDup(require);
...
int SessionExpiresRspDup(SipRsp rsp,SipSessionExpires *sessionexpires)
{
	...
	if(rsp->sessionexpires != NULL){
		...
		rsp->sessionexpires = NULL;
	}

	rsp->sessionexpires=SessionExpiresDup(sessionexpires);

	return 1;
}

int ServerHdrRspDup(SipRsp response,SipServer* server)
{
	if(NULL==response)
		return 0;
	...
	response->server=ServerHdrDup(server);
	return 1;
}
...
int SupportedRspDup(SipRsp response,SipSupported *support)
{
	...
	response->supported=SupportedDup(support);
...
int UnsupportedRspDup(SipRsp response,SipUnsupported *unsupport)
{
	...
	response->unsupported=UnsupportedDup(unsupport);
...
int UserAgentRspDup(SipRsp response,SipUserAgent* agent)
{
	...
	response->useragent = UserAgentDup(agent);
...
/* History-Info */
int	HistoryInfoRspDup(SipRsp response,const unsigned char* str)
{
	if(response->history_info != NULL)
		free(response->history_info);
	response->history_info=strDup(str);
	return 1;
}

/* P-Access-Network-Info */
int	PANIRspDup(SipRsp response,const unsigned char* str)
{
	if(response->paccessnetworkinfo != NULL)
		free(response->paccessnetworkinfo);
	response->paccessnetworkinfo=strDup(str);
	return 1;
}

/* P-Early-Media */
int	PEMRspDup(SipRsp response,const unsigned char* str)
{
	if(response->pearlymedia != NULL)
		free(response->pearlymedia);
	response->pearlymedia=strDup(str);
	return 1;
}

/* Reason */
int	ReasonRspDup(SipRsp response,const unsigned char* str)
{
	if(response->reason != NULL)
		free(response->reason);
	response->reason=strDup(str);
	return 1;
}

/* Session-ID */
int	SessionIDRspDup(SipRsp response,const unsigned char* str)
{
	if(response->sessionid != NULL)
		free(response->sessionid);
	response->sessionid=strDup(str);
	return 1;
}
