 ...
 * dx_hash.c
 *
 * $Id: dx_hash.c,v 1.2 2009/01/08 07:18:49 tyhuang Exp $
 */
/** 
* @file dx_hash.c
* @brief Define dx hash related functions
...
struct dxHashObj {
	...
	int		keysUpper_, strings_; /*strings_ indicates type*/
...
DxHash dxHashNew(int _initSize, int keysUpper, DxHashType strings)
{
	...
	_this = calloc(1,sizeof(struct dxHashObj));
	if( !_this )
		return NULL;
...
void* dxHashAdd(DxHash _this,const char* key, void* val)
{
	...
	if( e != NULL ) {
		...
		/* create new entry */
		/*e = calloc( sizeof *e );*/
		e=calloc(1,sizeof(struct entryObj));
		if( !e ) {
