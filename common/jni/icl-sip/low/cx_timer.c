 ...
 * cx_timer.c
 *
 * $Id: cx_timer.c,v 1.3 2008/11/12 03:46:45 tyhuang Exp $
 */
...
#ifdef _WIN32
#define TIME_RESOLUTION_MINISEC		50
...
#elif defined(_WIN32_WCE)
  #define MAX_CE_TIMER_THREAD		100
...
/** @brief  cxTimerInit -  allocate resources for cxTimer functions and initializes them. 
 *
 *  The cxTimerInit function allocate resources for cxTimer functions according the input parameter maxEvent, and set initial value to them.
 *
 *  @param maxEvents the maximum value of event buffers for timer.
...
RCODE cxTimerInit(int maxEvents) 
{
	...
	for( i=0; i<10; i++ ) {
		...
		/* print minTimer.it_value */
		TCRPrint(1,"minTime.it_value.tv_usec=%lu\n",minTime.it_value.tv_usec);

		if( minTime.it_value.tv_usec!=0 )
			break;
			
		if( i==9 && minTime.it_value.tv_usec==0 ) {
			TCRPrint(1,"cxTimerInit(): fail to get tick size!\n");
			return RC_ERROR;
		}
		/* marked by tyhuang 2008/11/11
		minTime.it_value = TV_ZERO;
		setitimer(ITIMER_REAL, &minTime, NULL);		
		*/
	}
#else
	/* for normal machine , usec should be 1 */
	minTime.it_value.tv_usec = 1;
#endif
...
/** @brief  cxTimerClean -  release all resources allocated in cxTimerInit function.
 *
 *  The cxTimerClean function release all resources allocated in cxTimerInit function and other cxTimer functions.
 *
 *  @return On success, RC_OK is returned.
 *          Otherwise, RC_ERROR is returned.
 */
...
#ifdef UNIX
	bRunTimerThread=FALSE;
	cxMutexFree(unixTimerMutex);
	cxThreadJoin(unixTimerThread);
#endif 
...
/** @brief  cxTimerHold - delay any incoming timers 
 *
 *  The cxTimerHold function delay any incoming timers until the cxTimerRelease function is called corresponding times.
 *
 */
void cxTimerHold(void)
{
...
/** @brief  cxTimerRelease - unblock delayed timer 
 *
 *  The cxTimerUnblock function release all delayed timer.
 */
void cxTimerRelease(void)
{
#if defined(UNIX)
...
/** @brief  decrTimer - descrease timeval value.
 *
 *  The cecrTimer function does subtraction for structure timeval. The function subtracts nextTime parameter from delta parameter and places the result back in delta parameter.
 *
 *  @param delta  the minuend.
 *  @param nextTime  the subtrahend.
 */
...
/** @brief  tickRound - smallest integral value which is multiple of tickSize and does not less than t
 *
 *  The tickRound function rounds the parameter t upwards to the nearest integral value  which is multiple of tickSize, and place the result back to the parameter t.
 *
 *  For example, when tickSize = 5, a value 2314 will round to 2315.
 *
 *  In general situation, tickSize is 1.
 *
 *  @param t the time value that want to be round
 */
...
static int resetTimer(int e, struct timeval delay, struct timeval *deltaP) 
{
	...
	*/
	...
	if( deltaP->tv_sec < 0	/* this is sooner than next event */
		...
		if( nTimeID==0 ) {
			TCRPrint(1, "resetTimer(): timeSetEvent() error!\n");
		#ifndef _WIN32_WCE
			assert(0);
		#endif
		}
...
static void scheduleTimer(int e, struct timeval delay) 
{
	...
	TCRPrint(100,"<scheduleTimer> (%d) delay = %d.%06d\n", e, delay.tv_sec, delay.tv_usec);
...
#endif
{
	...
	while( 1<=eventCount 
		...
		TCRPrint(100, "<doCB> (%d) delay.sec=%d, delay.usec=%d, eventCount = %d\n"
			   , heap[1]
...
/* @brief  heapSwap - exchange two records in heap array.
 *
 *  The heapSwap function exchanges data in two recrods of head array. 
 *
 *  @param a the index of the heap array.
 *  @param b the index of the heap array.
 */
/* indices are all into heap */
static void heapSwap(int a, int b)
{
...
/** @brief  cxTimerSetDeubg - set to debug mode
 *
 *  The cxTimerSetDebug function set cxTimer to debug mode.
 */
...
/** @brief  cxTimerResetDeubg - set to non-debug mode
 *
 *  The cxTimerResetDebug function set cxTimer routines to non-debug mode.
 */
...
/* @brief  heapCheck - check the order in event heaps
 *
 *  The heapCheck function checks the order in the event heaps, and print out error message in log files. HeapCheck function will do not thing but logging if error detected.
 *
 *  @param msg  the leading message for error message
 */
static void heapCheck(char* msg)
{
	int i;
...
void* unixTimer_thread(void* data)
{
	while (bRunTimerThread) {
		/* marked by tyhuang 2012/5/10 
		 * cxMutexLock(unixTimerMutex); 
		 */
