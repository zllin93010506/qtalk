 ...
 * sipTx.c
 *
 * $Id: sipTx.c,v 1.13 2009/03/04 06:45:48 tyhuang Exp $
 */
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
TTransactionDB_C * TxDatabase =NULL;
...
#define MAXTIMERCOUNT 65535 /* default is 65535 */
...
int GetStatusCode(SipRsp msg)
{
	...
	if(pStatus && pStatus->ptrStatusCode)
		return(atoi(pStatus->ptrStatusCode));
	else
		return 999;
}

unsigned int GetTxDBSize(void)
{
	if(NULL==TxDatabase)
		return 0;
	else
		return GetTransactionDBSize(&TxDatabase);
}
...
INT32 sipTxAddTimer(TxStruct TxData, SipTxEvtType TimerEvent)
{
...
void sipTxDelTimer(INT32 TimerHandle)
{
	void* tmp = NULL;
	...
	if ( RC_OK == CommonTimerDel( TimerHandle, &tmp ) )
					 ...
					 INT32 *timerid)
{
	...
	for (i=0; i < MAXTIMERCOUNT; i++)
	{
		if ( NULL == CommonTimerTable[i] )
		{
			_this = (CommonTimer) calloc( 1,sizeof(struct _CommonTimer) );
			_this->time = time;
			_this->cb = cb;
			_this->data = data;
...
RCODE CommonTimerDel(INT32 timerid, void** data)
{
	CommonTimer _this;
...
void _sipTxFree(TxStruct _this)
{
	...
	*/
	...
	if (NULL != _this->OriginalReq) {
		sipReqFree(_this->OriginalReq);
		_this->OriginalReq=NULL;
	}

	if (NULL != _this->Ack) {
		sipReqFree(_this->Ack);
		_this->Ack=NULL;
	}
	...
	if (-1 != _this->TimerHandle1) {
		sipTxDelTimer(_this->TimerHandle1);
		_this->TimerHandle1=-1;
	}
	
	if (-1 != _this->TimerHandle2) {
		sipTxDelTimer(_this->TimerHandle2);
		_this->TimerHandle2=-1;
	}

	if (-1 != _this->TimerHandle3) {
		sipTxDelTimer(_this->TimerHandle3);
		_this->TimerHandle3=-1;
	}
...
void CBSipTLSEventHandle(SipTLS tls, SipTLSEvtType event, SipMsgType msgtype, void* msg)
{
	...
	case SIP_TLSEVT_END:
		...
		if ( TxData ) {
			TxData->transport = NULL;
			TimerData = (TxTimer)calloc(1,sizeof(struct _TxTimer));
		...
		case SIP_MSG_REQ:
			if (msg != NULL) {
				...
				if ( TxData ) {
					...
					/* add by tyhuang 2007/7/25 */
					/* if request is ack, and tx status is not completed,
					 * drop it
					 */
					if((ACK==method) && (GetStatusCode(TxData->LatestRsp)<200)){
						TXTRACE0("[TLSEventHandle] WARNING: tx is not completed, discard ACK message \n");
						sipReqFree((SipReq)msg);
						msg = NULL;
						break;
					}
		...
		case SIP_MSG_REQ:
			if (msg != NULL) {
				...
				if ( TxData ) {
					 ...
					 */
					if((ACK==method) && (GetStatusCode(TxData->LatestRsp)<200)){
						TXTRACE0("[TCPEventHandle] WARNING: tx is not completed, discard ACK message \n");
						sipReqFree((SipReq)msg);
						msg = NULL;
						break;
					}
...
void CBSipUDPEventHandle(SipUDP udp, SipUDPEvtType event, SipMsgType msgtype,void* msg)
{
	...
	case SIP_UDPEVT_DATA:
		...
		case SIP_MSG_REQ:
			if (msg != NULL) {
				...
				if (TxData) {
					 ...
					 */
					if((ACK==method) && (GetStatusCode(TxData->LatestRsp)<200)){
						TXTRACE0("[UDPEventHandle] WARNING: tx is not completed, discard ACK message \n");
						sipReqFree((SipReq)msg);
						msg = NULL;
						break;
					}
		...
		case SIP_MSG_PING:
		case SIP_MSG_PONG:
			TXTRACE0("[UDPEventHandle] received sip ping-pong \n");
			break;
		default:
			break;
		}/* switch (msgtype) */
...
sipTxLibInit(SipTxEvtCB txEvtCB, SipConfigData ConfigStr, BOOL bMatchACK, unsigned short MaxTxCap)
{
	...
	*/
	...
	dxLstFree( AddrPortLst, (void (*)(void *))AddrPortFree );
...
*/
	...
	int pos;
	TxStruct tmpTx;
	count = dxLstGetSize(TimerQ);
	TXTRACE1("[sipTxLibEnd] Current TxList size = %d\n", count);
	for ( pos = count-1; pos >= 0; pos-- ){
		tmpTx = dxLstPeek(TimerQ, pos);
		tmpTx->state = TX_TERMINATED;
		_sipTxFree(tmpTx);
	}
		/***************/
...
CCLAPI RCODE sipTxEvtDispatch(int timeout)
{
	...
	i=dxLstGetSize(DispatchEvtQ);

	while ( i-- > 0  ) {
		evtobj = (TxDispatchEvtObj)dxLstGetHead( DispatchEvtQ );
		
		if(!evtobj)
			break;/*continue; 2008/8/15 by tyhuang */
		...
		case TX_DISPATCH_GARBAGE:
			...
			if (strcmp("REGISTER",sipMethodTypeToName(TxData->method))){
	...
	i=dxLstGetSize(TimerQ);
	while (i-- > 0 ) {
		
		TXTRACE1("[sipTxEvtDispatch] Process TimerQ : size = %d \n",i);

		TimerData = (TxTimer)dxLstGetHead(TimerQ);
		if (TimerData)
		{
...
CCLAPI UserProfile UserProfileDup(UserProfile profile)
{
	...
	_this = (UserProfile) calloc(1,sizeof (*_this));
	assert (_this);
	/*memset(_this, 0, sizeof (*_this));*/
...
CCLAPI unsigned short sipTxGetInternalNum(TxStruct _this)
{
	if (_this);
		return (unsigned short)_this->InternalNum;
