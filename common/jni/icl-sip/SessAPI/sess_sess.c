			  ...
			  IN UINT32 bandwidth)
{
	...
	if(bandwidth>0){
		SessAddBandwidth(se,DEFAUL_MODIFIER,bandwidth);
	}
...
RCODE SessFree(IN pSess *pSe)
{
	...
	if(se->bandwidthlist){
		dxLstFree(se->bandwidthlist,(void (*) (void*)) bwInfoFree);
	}
...
RCODE SessNewFromStr(OUT pSess* pSe,IN const char*sdp)
{
	...
	bdsize=sdpSessGetTbSize(sdpstruct);
	for(pos=0;pos<bdsize;pos++){
		if (RC_OK==sdpSessGetTbAt(sdpstruct,pos,&bandwidth)) {		
			if(RC_OK!=SessAddBandwidth(se,sdpTbModifer2Str(bandwidth.modifier_),bandwidth.bandwidth_)){
...
RCODE SessToStr(IN pSess se,OUT char *buffer,IN OUT INT32*length)
{
	...
	/* bandwidth */
	dxLstResetIter(se->bandwidthlist);
	while(NULL!=(tempbw=(pBWInfo)dxLstPeekByIter(se->bandwidthlist))){
		if(tempbw->modifier){
...
/* get Application Specific Maximum bandwidth of this session */
/* modifier is like : AS,CT,RR,RS */
/** \fn RCODE SessGetBandwidth(IN pSess se,IN const char* modifier,IN UINT32* value)
 *	\brief to get the bandwidth parameter
 *	\param se - a pointer to Sess object;NOT NULL
 *	\param modifier - bandwidth modifier is like : AS,CT,RR,RS
 *	\param value - return value of the bandwidth-value
 *	\retval RCODE RC_OK for success ;RC_ERROR for failure
 */
CCLAPI 
RCODE SessGetBandwidth(IN pSess se,IN const char* modifier,IN UINT32* value)
{
	pBWInfo temp;
	/* check parameter */
	if ( !se ) {
		SESSWARN ("[SessGetBandwidth] Invalid argument : Null pointer to sessmedia\n" );
		return RC_ERROR;
	}
	if (!modifier) {
		SESSWARN ("[SessGetBandwidth] Invalid argument : Null pointer to modifier\n" );
		return RC_ERROR;
	}
	if (!value) {
		SESSWARN ("[SessGetBandwidth] Invalid argument : Null pointer to value\n" );
		return RC_ERROR;
	}

	dxLstResetIter(se->bandwidthlist);
	while(NULL!=(temp=(pBWInfo)dxLstPeekByIter(se->bandwidthlist))){
		if(strICmp(temp->modifier,modifier)==0){
			*value=temp->value;
			return RC_OK;
		}
	}
...
/** \fn RCODE SessAddBandwidth(IN pSess se,IN const char* modifier,IN UINT32 value)
 *	\brief to add the bandwidth parameter ("b=rr:bandwidth-value")
 *	\param se - a pointer to Sess object;NOT NULL
 *	\param modifier - bandwidth modifier is like : AS,CT,RR,RS
 *	\param value - the bandwidth-value will be set 
 *	\retval RCODE RC_OK for success ;RC_ERROR for failure
 */
CCLAPI 
RCODE SessAddBandwidth(IN pSess se,IN const char* modifier,IN UINT32 value)
{
	pBWInfo b=NULL;
	/* check parameter */
	if ( !se ) {
		SESSWARN ("[SessAddBandwidth] Invalid argument : Null pointer to sessmedia\n" );
		return RC_ERROR;
	}
	if (!modifier) {
		SESSWARN ("[SessAddBandwidth] Invalid argument : Null pointer to modifier\n" );
		return RC_ERROR;
	}

	b=(pBWInfo)calloc(1,sizeof (struct BWobj));
	if(!b){
		SESSERR("[SessAddBandwidth] memory alloc failed!");
		return RC_ERROR;
	}
	...
	if(!se->bandwidthlist)
		se->bandwidthlist=dxLstNew(DX_LST_POINTER);
	
	return dxLstPutTail(se->bandwidthlist,(void *)b);
}

CCLAPI 
RCODE SessSetBandwidth(IN pSess se,IN const char* modifier,IN UINT32 value)
{
	pBWInfo temp;
	/* check parameter */
	if ( !se ) {
		SESSWARN ("[SessSetBandwidth] Invalid argument : Null pointer to sessmedia\n" );
		return RC_ERROR;
	}
	if (!modifier) {
		SESSWARN ("[SessSetBandwidth] Invalid argument : Null pointer to modifier\n" );
		return RC_ERROR;
	}
	...
	return SessAddBandwidth(se,modifier,value);	
}
...
CCLAPI 
RCODE SessSetOwnerAddress(IN pSess se,IN const char *owneraddr)
{
	/* check parameter */
	if ( !se ) {
		SESSWARN ("[SessSetOwner] Invalid argument : Null pointer to sess\n" );
