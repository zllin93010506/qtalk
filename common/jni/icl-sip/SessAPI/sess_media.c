				   ...
				   IN pMediaPayload rtpmap)
{
	...
	if (port>65535) {
		SESSWARN ("[SessMediaSetPort] Invalid argument : port should in the range 0 to 65535 inclusive\n" );
		/* return RC_ERROR; */
	}
	if (!rtpmap) {
...
{
	pSessMedia media=NULL,newmedia=NULL;
	pMediaPayload pl,newpl;
	INT32 pos,size;
	
	/* check parameter */
	if ( !pMedia ) {
		SESSWARN ("[SessMediaDup] Invalid argument : Null pointer to sessmedia\n" );
		return RC_ERROR;
	}
	media=*pMedia;
	if ( !media ) {
	...
	/* memory alloc */
	newmedia=(pSessMedia)calloc(1,sizeof(SessMedia));
	if (!newmedia) {
		SESSERR("[SessMediaDup] Memory allocation error\n" );
		return RC_ERROR;
	}

	newmedia->type=media->type;
	newmedia->address=strDup(media->address);
	newmedia->transport=strDup(media->transport);
	newmedia->port=media->port;
	newmedia->numofport=media->numofport;
	newmedia->ptime=media->ptime;
	newmedia->status=media->status;

	if (media->rtpmaplist) {
		size=dxLstGetSize(media->rtpmaplist);
		newmedia->rtpmaplist=dxLstNew(DX_LST_POINTER);
		for(pos=0;pos<size;pos++){
			pl=dxLstPeek(media->rtpmaplist,pos);
				
			newpl=NULL;
			MediaPayloadDup(pl,&newpl);
			dxLstPutTail(newmedia->rtpmaplist,(void *)newpl);
		}
	}
	if (media->fmtplist) {
		size=dxLstGetSize(media->fmtplist);
		newmedia->fmtplist=dxLstNew(DX_LST_POINTER);
		for(pos=0;pos<size;pos++){
			pl=dxLstPeek(media->fmtplist,pos);
				
			newpl=NULL;
			MediaPayloadDup(pl,&newpl);
			dxLstPutTail(newmedia->fmtplist,(void *)newpl);
		}
	}

	if(media->bandwidthlist){
		pBWInfo oldb=NULL,newb=NULL;

		newmedia->bandwidthlist=dxLstNew(DX_LST_POINTER);
		size=dxLstGetSize(media->bandwidthlist);
		for(pos=0;pos<size;pos++){
			newb=(pBWInfo)calloc(1,sizeof (struct BWobj));

			oldb=(pBWInfo)dxLstPeek(media->bandwidthlist,pos);
			if(oldb && newb){
				newb->modifier=strDup(oldb->modifier);
				newb->value=oldb->value;

				dxLstPutTail(newmedia->bandwidthlist,(void *)newb);
			}
		}
	}

	if(media->attribelist){
		pMediaAtt olda=NULL,newa=NULL;

		newmedia->attribelist=dxLstNew(DX_LST_POINTER);
		size=dxLstGetSize(media->attribelist);
		for(pos=0;pos<size;pos++){
			newa=(pMediaAtt)calloc(1,sizeof (struct Attobj));

			olda=(pMediaAtt)dxLstPeek(media->attribelist,pos);
			if(olda && newa){
				newa->name=strDup(olda->name);
				newa->value=strDup(olda->value);

				dxLstPutTail(newmedia->attribelist,(void *)newa);
			}
		}
	}

	if(media->cryptolist){
		newmedia->cryptolist=dxLstNew(DX_LST_STRING);
		size=dxLstGetSize(media->cryptolist);
		for(pos=0;pos<size;pos++){
			dxLstPutTail(newmedia->cryptolist,dxLstPeek(media->cryptolist,pos));
		}
	}
...
CCLAPI 
RCODE SessMediaSetBandwidth(IN pSessMedia media,IN const char* modifier,IN UINT32 value)
{
	pBWInfo temp=NULL;
	/* check parameter */
	if ( !media ) {
	...
	return SessMediaAddBandwidth(media,modifier,value);
}


/* get Application attribute of this media */
CCLAPI 
...
RCODE SessMediaNewFromSdp(OUT pSessMedia *pMedia,IN SdpMsess msess,IN StatusAttr sessstatus)
{
	...
	*/
	...
	media->transport=strDup(MediaAnn.protocol_name_);
