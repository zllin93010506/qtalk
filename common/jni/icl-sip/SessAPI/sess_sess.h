...
CCLAPI RCODE SessSetTool(IN pSess,IN const char*);
/* get sess tool attribute */
CCLAPI RCODE SessGetTool(IN pSess,OUT char*buffer,IN OUT INT32*length);

/* get Application Specific Maximum bandwidth of this session */
/* modifier is like : AS,CT,RR,RS */
CCLAPI RCODE SessGetBandwidth(IN pSess se,IN const char* modifier,IN UINT32* value);
CCLAPI RCODE SessAddBandwidth(IN pSess se,IN const char* modifier,IN UINT32 value);
CCLAPI RCODE SessSetBandwidth(IN pSess se,IN const char* modifier,IN UINT32 value);

/* set/get sess owner */
CCLAPI RCODE SessSetOwner(IN pSess,IN const char*);
CCLAPI RCODE SessGetOwner(IN pSess,OUT char*buffer,IN OUT INT32*length);
CCLAPI RCODE SessSetOwnerAddress(IN pSess se,IN const char *owneraddr);
