...
#include <jni.h>
#include "logging.h"
#include "include/AecSpeakerTransformJni.h"
...
/* =============================================================================
                                    Macros
   ============================================================================= */
#define TAG "QSE_AecSpeakerTransformJni"
...
NativeParams* result = (NativeParams*) malloc(sizeof(NativeParams));
	...
	sw_aec_init(sampleRate,result->jvm,jclass_caller);
#else
...
JNIEXPORT void JNICALL Java_com_quanta_qtalk_media_audio_AecSpeakerTransform__1OnMedia
  (JNIEnv * env, jclass jclass_caller, jint params, jshortArray jobject_input, jint length, jlong timestamp) {
	...
	if (handle != NULL && jobject_input != NULL)
	{
		...
		if(length>(handle->sampleRate)/50 && (handle->sampleRate)!=0){
			length = (handle->sampleRate)/50;
		}
		sw_aec_push((char*)array_input, 2*length);
