...
BOOL BeDestroyUsage(int statuscode);
...
RCODE ProcessReq(pSipDlg *pdlg,TxStruct txData,int beAck,pSipDlg *rpdlg)
{
	...
	if(!dlg){
		...
		if((method==SIP_NOTIFY)||(method==BYE)||(method==CANCEL)){
			...
			*/
			if(RC_OK!=sipDlgNew(&dlg,callid,localurl,remoteurl,NULL,NULL)){
...
RCODE ProcessRsp(TxStruct txData)
{
	...
	if( remotetag ) {
		if( totag ){
			/* add check for invite and dialog status */
			if((state>DLG_STATE_EARLY)&&(strCmp(remotetag,totag)!=0)){
	...
	if(totag){
		...
		if(DLG_STATE_EARLY==state){
			...
			DLGWARN("[ProcessRsp] remotetag is updated by new response");
			dlg->remotetag=strDup(totag);
		}	
	}
	...
	if((remotetarget)&&(rspcode!=401)&&(rspcode!=407)&&(rspcode!=100)){
		if(dlg->remotetarget){
			free(dlg->remotetarget);
		}
	...
	if(method==INVITE){
		...
		if(Be100rel(rsp)){
			...
			if((rseq > 0) && ((UINT32)rseq > dlg->remoterseq)){
			/* if(rseq > 0){ */
				char rackbuf[64];	
				SipCSeq *cseqhdr=(SipCSeq *)sipRspGetHdr(rsp,CSeq);
				
				memset(rackbuf,0,64*sizeof(char));
	...
	if(BeDestroyUsage(rspcode)&&(DLG_STATE_CONFIRM==dlg->state)){
		/*
		switch(method){
		case SIP_SUBSCRIBE:
		case REFER:
	...
	/* if there is no usage in dialog,state will be change to terminated */
	if(dxLstGetSize(dlg->eventlist)==0){
				dlg->state=DLG_STATE_TERMINATED;
	}
...
void TxEvtCB(TxStruct txData, SipTxEvtType event)
{
	...
	switch (event) {
		...
		case TX_TIMER_A:
		case TX_TIMER_E:
			/* if (txData && (3==txData->retrytime )) */
			if (txData && (g_tx_etime_interval==txData->retrytime ))
			{
				/* fire time e event to upper layer */
				/* post event message */
				if(sipDlgMsgNew(&msg,NULL,tmpReq,NULL,(void*)txData)== RC_OK){
					if(sipDlgPostMsg(DLG_MSG_TX_ETIME,msg)!= RC_OK)
						DLGERR("[TxEvtCB] post msg failed!");
				}else
					DLGERR("[TxEvtCB] create post msg failed!");
				}
...
sipDlgInit(IN DlgEvtCB eventCB,IN SipConfigData config)
{
	...
	g_useragent=strDup(SIP_USER_AGENT);
...
CCLAPI 
RCODE 
sipDlgNew(pSipDlg *pdlg,const char *callid,const char *localurl,const char *remoteurl,DxLst routeset,IN const char *localtag)
{
	pSipDlg dlg=NULL;
	/* check parameter */
	if ( !DLGDBLst) {
...
sipDlgGetCallID(pSipDlg *const pdlg,char *callid ,INT32 *len)
{
	...
	INT32 length = (INT32)strlen(dlg->callid);
	if(*len==0){
		*len=length;
		return RC_ERROR;
	}
...
sipDlgGetLocalURL(pSipDlg *const pdlg,char *localurl,INT32 *len)
{
	...
	INT32 length = strlen(dlg->localurl);
	if(*len==0){
		*len=length;
		return RC_ERROR;
	}
	...
	/* clean buffer */
	memset(localurl,0,*len);
	/* if buffer size is enough ,copy local url to buffer */
	if(length< *len ){
		strncpy(localurl,dlg->localurl,length);
		*len=length;
		return RC_OK;
	}else
		return RC_ERROR;
...
sipDlgGetLocalTag(pSipDlg *const pdlg,char *localtag,INT32 *len)
{
	...
	INT32 length = strlen(dlg->localtag);
	if(*len==0){
		*len=length;
		return RC_ERROR;
	}
...
sipDlgGetRemoteURL(pSipDlg *const pdlg,char *remoteurl,INT32 *len)
{
	...
	INT32 length= strlen(dlg->remoteurl);
	if(*len==0){
		*len=length+1;
		return RC_ERROR;
	}
	/* clean buffer */
	memset(remoteurl,0,*len);
	/* if buffer size is enough ,copy remote url to buffer */
	if(length< *len ){
		strncpy(remoteurl,dlg->remoteurl,*len);
		*len=length;
		return RC_OK;
	}else
		return RC_ERROR;
...
sipDlgGetRemoteTag(pSipDlg *const pdlg,char *remotetag,INT32 *len)
{
	...
	INT32 length=strlen(dlg->remotetag);
	if(*len==0){
		*len=length;
		return RC_ERROR;
	}
	...
	/* clean buffer */
	memset(remotetag,0,*len);
	/* if buffer size is enough ,copy remote tag to buffer */
	if(length<= *len ){
		strncpy(remotetag,dlg->remotetag,length);
		*len=length;
		return RC_OK;
	}else
		return RC_ERROR;
...
sipDlgGetRemoteTarget(pSipDlg *const pdlg,char *remotetarget,INT32 *len)
{
	...
	INT32 length = strlen(dlg->remotetarget);
	if(*len==0){
		*len=length;
		return RC_ERROR;
	}
...
sipDlgNewReq(pSipDlg *const pdlg,SipReq *preq,SipMethodType method)
{
	...
	if(PRACK==method){
		...
		if(NULL!=rackhdr){
			...
			/* 
				2010/8/27 for 401/407,
				but user must add RAck from old PRACK request by himself.
			*/

			/* sipReqFree(req); */
			DLGWARN("[sipDlgNewReq] Not RSeq is found for PRACK request!\n" );
			/* return RC_ERROR; */
		}	
	}
...
sipDlgSetAllowEvent(IN const char *ae)
{
	...
	if (!ae) {
		DLGWARN("[sipDlgSetAllowEvent] Invalid argument : Null pointer to allow-events\n" );
		return RC_ERROR;
	}
...
RCODE 
sipDlgSetUserAgent(IN const char *ua)
{
	RCODE rcode=RC_OK;
	char *tmp;

	if (!ua) {
		DLGWARN("[sipDlgSetUserAgent] Invalid argument : Null pointer to allow-events\n" );
		return RC_ERROR;
	}

	tmp=g_useragent;
	g_useragent=strDup(ua);
	if (NULL!=g_useragent) {
		free(tmp);
		rcode=RC_OK;
	}else{
...
RCODE sipDlgSetTxEtimeInterval(UINT8 newvalve)
{
	...
	if(newvalve>0 && newvalve < 9)
	{
		g_tx_etime_interval=newvalve;
		return RC_OK;
	}else{
		return RC_ERROR;
	}
