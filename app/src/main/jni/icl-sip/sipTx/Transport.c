 ...
 * Transport.c
 *
 * $Id: Transport.c,v 1.5 2009/03/04 06:45:48 tyhuang Exp $
 */
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
void FinalizeTransport ()
{
	...
	if (sipUDPTable) {
		dxHashFree(sipUDPTable, (void (*)(void *))sipUDPFree);
		sipUDPTable = NULL;
	}
	...
	if (sipTCPTable) {
		/*
		 * modified by tyhuang
		 * dxHashFree(sipTCPTable, (void (*)(void *))sipUDPFree);
		 */
		dxHashFree(sipTCPTable, (void (*)(void *))sipTCPFree);
		sipTCPTable = NULL;
	}
}
...
char * CalculateBranch ( SipReq pMsg ) {
	...
	static unsigned int gencount=1;
...
RCODE SendResponse2 ( TxStruct TxData ) {
	...
	*/
	...
	if ( 0 == strICmp ( pVia->transport, "UDP" ) ) {
...
RCODE SendPingPong(const char* ip,UINT16 port,UINT8 transport,BOOL bPing)
{
	char msg[5]="\0";
	...
	if ( 0 ==transport ) {
		/* udp */
		if ( sipUDPSendTxt ( g_sipUDP, msg , (char *)ip, port ) ) {
			TXTRACE0("[TRANSPORT] UDP sent successful \n");
			return RC_OK;
		} else {
			TXERR0("[TRANSPORT] Sent thru UDP fail !!! \n");
