...
int x264_encoder_headers( x264_t *h, x264_nal_t **pp_nal, int *pi_nal )
{
    ...
    /* identify ourselves */    
    if (h->param.b_version_info)//fish
    {
                             ...
                             x264_picture_t *pic_out )
{
    ...
    *pp_nal = NULL;
    ...
    if(h->param.b_version_info)//fish
    {
        ...
        if( h->fenc->b_keyframe && h->param.b_version_info)//fish
        {
                                   ...
                                   x264_picture_t *pic_out )
{
    ...
    if( h->fenc->b_keyframe && h->sps->vui.b_nal_hrd_parameters_present && h->param.b_version_info)//fish
    {
        x264_hrd_fullness( h );
        x264_nal_start( h, NAL_SEI, NAL_PRIORITY_DISPOSABLE );
