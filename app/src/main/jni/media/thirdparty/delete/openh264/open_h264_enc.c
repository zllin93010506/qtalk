...
int open_h264_encorder_create()
{
    ...
    if (NULL == handle) {
        LOGE("H264Handle malloc failure");
        return -1;
    } else {
                                  ...
                                  const int gop, H264DataFp cb)
{
    ...
    LOGE("%s(), %d fps, width:%d, height:%d, bit_rate_bps:%d",  __FUNCTION__, fps, width, height, bit_rate_bps);
 ...
 //       handle->h264_data_cb = cb;
    ...
    (*svc_encorder)->GetDefaultParams (svc_encorder,&param);
    ...
    if (svc_encorder) {
        ret = (*svc_encorder)->InitializeExt(svc_encorder, &param);
    }
    LOGE("%s(), ret = %d", __FUNCTION__, ret);

    //unsigned int uiTraceLevel = WELS_LOG_ERROR;
    //(*svc_encorder)->SetOption (svc_encorder,ENCODER_OPTION_TRACE_LEVEL, &uiTraceLevel);

    ret = (*svc_encorder)->SetOption (svc_encorder,ENCODER_OPTION_DATAFORMAT, &videoFormat);
    LOGE("%s(), SetOption ret = %d", __FUNCTION__, ret);
...
int open_h264_encorder_uninitialize()
{
    ...
    if (svc_encorder) {
        ret = (*svc_encorder)->Uninitialize(svc_encorder);
    }
    LOGE("%s ret = %d", __FUNCTION__, ret);
                                    ...
                                    SFrameBSInfo*   bsInfo)
{
    int ret = -1;
    if (svc_encorder) {
        ret = (*svc_encorder)->EncodeFrame(svc_encorder, pic, bsInfo);
    }
    return ret;
}
...
int open_h264_encodeframe2(unsigned char *yuvdata, int yuv_size, uint32_t timestamp, char *h264out)
{
    ...
    LOGE("%s(), yuv:%p, size:%d, width:%d, height:%d", __FUNCTION__, yuvdata, yuv_size, handle->width, handle->height);

    memset(&bsInfo, 0x0, sizeof(SFrameBSInfo));
    ret = (*svc_encorder)->EncodeFrame(svc_encorder, &pic, &bsInfo);
    if(ret < 0) {
        LOGE("%s <error> EncodeFrame failed, ret: %d", TAG, ret);
        return -1;
    }
...
int open_h264_encorder_set_bitrate(const int bps)
{
    ...
    if (svc_encorder) {
    	...
    	Info.iBitrate = bps;
    	g_bitrate = bps;
    	ret = (*svc_encorder)->SetOption(svc_encorder, ENCODER_OPTION_BITRATE,
                                         (void*)&Info);
    }
    LOGE("%s(), bps = %d ret = %d", __FUNCTION__, bps, ret);
    return ret;
}
...
int open_h264_encorder_set_frame_rate(const int frame_rate)
{
    ...
    if (svc_encorder) {
    	...
    	(*svc_encorder)->GetOption(svc_encorder, ENCODER_OPTION_FRAME_RATE,
    	    	                                   &get_fr);
    	//LOGE("before set frame rate: %f",get_fr);
        ret = (*svc_encorder)->SetOption(svc_encorder, ENCODER_OPTION_FRAME_RATE,
                                   &set_fr);
    	(*svc_encorder)->GetOption(svc_encorder, ENCODER_OPTION_FRAME_RATE,
    	    	                                   &get_fr);
    	//LOGE("after set frame rate: %f",get_fr);
    }

    LOGE("%s(), fps = %d ret = %d", __FUNCTION__, frame_rate, ret);
...
int open_h264_encorder_idr_request()
{
    ...
    if (svc_encorder) {
        ret = (*svc_encorder)->ForceIntraFrame(svc_encorder, true);
    }
    LOGE("%s(), ret = %d", __FUNCTION__, ret);
