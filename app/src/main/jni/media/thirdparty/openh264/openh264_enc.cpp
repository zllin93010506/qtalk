...
int open_h264_encorder_create()
{
    ...
    if (NULL == handle) {
        LOGE("QSE: <error> H264Handle malloc failure");
        return -1;
    }
    else {
                                  ...
                                  const int gop, H264DataFp cb)
{
    ...
    LOGD("%s(), %d fps, width:%d, height:%d, kbps:%d",  __FUNCTION__, fps, width, height, kbps);
    ...
    param.iTargetBitrate = kbps * 1000;
    param.iMaxBitrate = kbps * 1000;
    param.iRCMode = RC_BITRATE_MODE; // RC_QUALITY_MODE ; RC_BITRATE_MODE
    ...
    /* rc control */
    param.bEnableFrameSkip = false; /* 當bitrate不足時，丟棄當前這個frame */
    param.iMaxQp = 50; //40;
    param.iMinQp = 0;  //16;
    ...
    param.bEnableAdaptiveQuant = false; // true
    param.bEnableFrameCroppingFlag = false;
    param.bEnableSceneChangeDetect = true; // false
...
int open_h264_encodeframe(unsigned char *yuvdata, int yuv_size, uint32_t timestamp, unsigned char *h264out)
{
    ...
    if((ret < 0) || (bsInfo.eFrameType == videoFrameTypeSkip) || (bsInfo.eFrameType == videoFrameTypeInvalid)) {
        LOGE("QSE: Drop openh264 encode frame, ret:%d, eFrameType:%d (fk)", ret, bsInfo.eFrameType);
        return -1;
    }
...
int open_h264_encorder_set_bitrate(const int kbps)
{
    int ret = -1, low_kbps_threshold = 0;
    bool frame_skip = false;
    SBitrateInfo target_bitrate;

    if (svc_encorder) {
        //memset(&target_bitrate, 0, sizeof(SBitrateInfo));
        svc_encorder->GetOption(ENCODER_OPTION_BITRATE, &target_bitrate);
    	target_bitrate.iLayer = SPATIAL_LAYER_ALL;
    	target_bitrate.iBitrate = kbps * 1000; // kbps -> bps
    	ret = svc_encorder->SetOption(ENCODER_OPTION_BITRATE, &target_bitrate);

        low_kbps_threshold = (int)((float)handle->width * (float)handle->height * (float)0.0011);
        if(kbps < low_kbps_threshold) { // enable skip frame
            frame_skip = true;
        }
        ret = svc_encorder->SetOption(ENCODER_OPTION_RC_FRAME_SKIP, &frame_skip);
    }
